...
if ($_POST['hostname'])
{
  if ($_SESSION['userlevel'] > '5')
  {
    ...
    {
      print_error("不支持的协议版本, 请在下拉菜单中选择您的错误路线 ?");
    }
    ...
    if ($result)
    {
      print_success("设备添加完成 (id = $result)");
    }
  } else {
    print_error("您没有添加主机的必要权限.");
  }
}
...
<form name="form1" method="post" action="" class="form-horizontal">
  ...
  <p>设备将被进行Ping和SNMP的可达性检测. 只有确认操作系统的设备将会被添加.</p>
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <label class="checkbox">
          <input type="checkbox" name="ignorerrd" value="confirm" <?php if ($config['rrd_override']) { echo('disabled checked'); } ?> />添加设备如果RRD目录已经存在
        </label>
      </div>
    </div>

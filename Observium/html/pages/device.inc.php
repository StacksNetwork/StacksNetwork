...
if (isset($cache['devices']['id'][$vars['device']]) || $permit_ports)
{
  ...
  if (device_permitted($device['device_id']))
  {
    ...
    {
      $navbar['options']['overview'] = array('text' => '概述', 'icon' => 'oicon-server');
    }

    $navbar['options']['graphs'] = array('text' => '流量图', 'icon' => 'oicon-chart-up');
    ...
    {
      $navbar['options']['apps'] = array('text' => '应用', 'icon' => 'oicon-application-icon-large');
    }
    ...
    {
      $navbar['options']['collectd'] = array('text' => '汇总', 'icon' => 'oicon-chart-up-color');
    }
    ...
    {
      $navbar['options']['ports'] = array('text' => '端口', 'icon' => 'oicon-network-ethernet');
    }
          ...
          if (basename($file) != "." && basename($file) != "..")
          {
            if (strstr($file, ".rrd"))
            {
              if (strstr($file, "~"))
              {
                ...
                if ($config['smokeping']['suffix']) $target = $target.$config['smokeping']['suffix'];
    ...
    {
      $navbar['options']['loadbalancer'] = array('text' => '负载均衡', 'icon' => 'oicon-arrow-split');
    }
    ...
    {
      $navbar['options']['routing'] = array('text' => '路由', 'icon' => 'oicon-arrow-branch-000-left');
    }
    ...
    {
      $navbar['options']['pseudowires'] = array('text' => '伪流量', 'icon' => 'oicon-layer-shape-curve');
    }
...
#    }
    ...
    {
      $navbar['options']['services'] = array('text' => '服务', 'icon' => 'oicon-target');
    }
    ...
    {
      $navbar['options']['printing'] = array('text' => '打印', 'icon' => 'oicon-printer-color');
    }
    ...
    {
      $navbar['options']['alerts'] = array('text' => '报警', 'icon' => 'oicon-bell');
    }
    ...
    {
      $navbar['options']['showconfig'] = array('text' => '设置', 'icon' => 'oicon-application-terminal');
    }
    ...
    if ($nfsen_rrd_file)
    {
      $navbar['options']['nfsen'] = array('text' => '网络流量', 'icon' => 'oicon-funnel');
    }

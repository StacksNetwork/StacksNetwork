...
<svg width="100%" height="100%" viewBox="0 0 <?php echo("$width $height") ?>" preserveAspectRatio="none" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" onload="init(evt)">
  <g id="graph">
    ...
    <text id="switch_unit" x="<?php echo($width*0.55) ?>" y="5" <?php echo($attribs['switch_unit']) ?>>切换到 bytes/s</text>
    <text id="switch_scale" x="<?php echo($width*0.55) ?>" y="11" <?php echo($attribs['switch_scale']) ?>>自动调整(<?php echo($scale_type) ?>)</text>
    <text id="datetime" x="<?php echo($width*0.33) ?>" y="5" <?php echo($attribs['legend']) ?>> </text>
    <text id="graphlast" x="<?php echo($width*0.55) ?>" y="17" <?php echo($attribs['legend']) ?>>曲线图显示最近 <?php echo($time_interval*$nb_plot) ?> 秒</text>
    ...
    <text id="collect_initial" x="<?php echo($width*0.5) ?>" y="<?php echo($height*0.5) ?>"  visibility="hidden" <?php echo($attribs['collect_initial']) ?> text-anchor="middle">原始数据的收集, 请稍等...</text>
  </g>
...
{
  SVGDoc.getElementById('switch_unit').firstChild.data = '切换到 ' + unit + '/s';
...
function switch_scale(event)
{
  ...
  SVGDoc.getElementById('switch_scale').firstChild.data = '自动调整 (' + scale_type + ')';
}

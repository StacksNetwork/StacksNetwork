...
function collectd_flush($identifier) {
	...
	if ($socket = @fsockopen($config['collectd_sock'], 0, $u_errno, $u_errmsg)) {
		...
		if ($r === false || $r != strlen($cmd))
			error_log(sprintf("graph.php: 无法写入完整命令到 unix-socket: %d 超出 %d 已写入", $r === false ? -1 : $r, strlen($cmd)));

		$resp = fgets($socket);
		if ($resp === false)
			error_log(sprintf("graph.php: 未能读取响应部分的命令: %s", trim($cmd)));
		...
		error_log(sprintf("graph.php: 无法打开 unix-socket 部分: %d: %s", $u_errno, $u_errmsg));
}

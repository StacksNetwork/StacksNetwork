...
$rewrite_iftype = array(
  ...
  'e1' => 'E1',
  'basicISDN' => '基础速率ISDN',
  'primaryISDN' => '基本速率ISDN',
  'propPointToPointSerial' => 'PtP系列',
  'ppp' => 'PPP',
  ...
  'propMultiplexor' => '所有权多路复用',
  'ieee80212' => '100BaseVG',
  'fibreChannel' => '光纤通道',
  'hippiInterface' => 'HIPPI',
  'frameRelayInterconnect' => '帧中继',
...
function rewrite_unix_hardware($descr, $hw = NULL)
{
  ...
  elseif (stristr($descr, 'ppc'))     { $hardware .= ' [PPC]'; }
...
function short_ifname($if, $len = NULL)
{
  $len = (is_numeric($len) ? (int)$len : FALSE);

  $if = rewrite_ifname($if);
  $if = strtolower($if);

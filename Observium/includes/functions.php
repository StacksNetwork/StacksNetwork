...
function messagebus_send($message)
{
  ...
  if ($socket = socket_create(AF_INET, SOCK_DGRAM, SOL_UDP))
  {
    ...
    print("对AQMP代理服务器无法创建UDP Socket.".PHP_EOL);
    return FALSE;
  }
...
function get_device_os($device)
{
  ...
  if ($recheck)
  {
    ...
    if ($file)
    {
      ...
      print_warning("OLD: $old_os, NEW: $os ---");
      if ($os && $os == $old_os) { return $old_os; }
    }
    ...
    if (gethostbyname6($new, TRUE))
    {
      ...
      if (isPingable($new))
      {
        ...
        if (!file_exists($config['rrd_dir'].'/'.$new))
        {
          ...
          {
            print_error("未能重命名. 重命名 RRD 目录时发生错误.");
            return FALSE;
          }
          $return = dbUpdate(array('hostname' => $new), 'devices', 'device_id=?', array($id));
          log_event("主机名变更: $host -> $new ($source)", $id, 'system');
          return TRUE;
        } else {
          // directory already exists
          print_error("未能重命名. 目录 rrd/$new 已经存在");
        }
      } else {
        // failed Reachability
        print_error("未能重命名. 无法ping $new");
      }
    } else {
      // Failed DNS lookup
      print_error("未能重命名. 无法解析 $new");
    }
  } else {
    // found in database
    print_error("未能重命名. 已经获得主机 $new");
  }
  return FALSE;
}
...
function check_device_duplicated($device)
{
  ...
  if (!empty($snmpEngineID))
  {
    ...
    if ($sysName !== FALSE && dbFetchCell('SELECT COUNT(*) FROM `devices` WHERE `disabled` = 0 AND `sysName` = ?', array($sysName)) > 0)
    {
      ...
      print_error("已有设备使用 SNMP-read sysName ($sysName).");
      return TRUE;
    }
  }
...
function isSNMPable($device)
{
  ...
  $pos = snmp_get($device, '.1.3.6.1.2.1.1.3.0', '-Oqtn', 'DISMAN-EVENT-MIB', mib_dirs()); // sysUpTime
...
function createHost($host, $community = NULL, $snmpver, $port = 161, $transport = 'udp', $v3 = array())
{
  ...
  if ($device['os'])
  {
    ...
    if ($device_id)
    {
      log_event("Device added: $host", $device_id, 'system');
      if (is_cli())
      {
...
function log_event($text, $device = NULL, $type = NULL, $reference = NULL)
{
  ...
  if ($device['ignore']) { return FALSE; } // Do not log events if device ignored
  if ($type == 'interface')
  {
...
function is_port_valid($port, $device)
{
  ...
  if ($valid && empty($port['ifDescr']) && empty($port['ifName'])) { $valid = FALSE; } /// FIXME. Why?
  if ($valid && $device['os'] == 'catos' && strstr($if, "vlan"))   { $valid = FALSE; } /// FIXME. Why?
...
function state_string_to_numeric($type, $value)
{
  ...
  {
    if ($content['name'] == $value) { return $index; }
  }

...
foreach ($wmi['mssql']['services'] as $instance)
{
  ...
  if ($wmi['mssql'][$instance['Name']]['stats'])
  {
    ...
    $rrd_filename = $GLOBALS['config']['rrd_dir'] . "/" . $device['hostname'] . "/wmi-app-mssql_".$instance['Name']."-stats.rrd";
  ...
  if ($wmi['mssql'][$instance['Name']]['memory'])
  {
    ...
    $rrd_filename = $GLOBALS['config']['rrd_dir']."/".$device['hostname']."/wmi-app-mssql_".$instance['Name']."-memory.rrd";
    if (!is_file($rrd_filename))
    {
      rrdtool_create($rrd_filename,

...
/**
 * REF is a nicer alternative to PHP's print_vars() / var_dump().
 *
 * @version  1.0
...
class ref{
  ...
  public function __construct($format = 'html'){
    ...
    if($format instanceof RFormatter){
      ...
      if(!class_exists($format))
        throw new \Exception(sprintf('%s 类未找到', $format));
  ...
  public function __get($name){
    throw new \Exception(sprintf('无该属性: %s', $name));
  }

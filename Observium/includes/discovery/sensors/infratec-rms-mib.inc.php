...
foreach ($tempoid as $key => $dummy)
{
  ...
  if (is_numeric($value) && $value < 600)
  {
    ...
    discover_sensor($valid['sensor'], 'temperature', $device, $tempoid[$key]['value'], $key, 'infratec-rms', $descr, 1, 1, NULL, NULL, NULL, NULL, $value);
  }
}
...
foreach ($humoid as $key => $dummy)
{
  ...
  if (is_numeric($value) && $value <= 100)
  {
    ...
    $descr = preg_replace("/\"/","",$descr);
    discover_sensor($valid['sensor'], 'humidity', $device, $humoid[$key]['value'], $key, 'infratec-rms', $descr, 1, 1, NULL, NULL, NULL, NULL, $value);
  }
}
...
foreach ($mainsoid as $key => $dummy)
{
  ...
  if (is_numeric($value) && $value > 0)
  {
    ...
    $descr = preg_replace("/\"/","",$descr);
    discover_sensor($valid['sensor'], 'voltage', $device, $mainsoid[$key]['value'], $key, 'infratec-rms', $descr, 1, 1, NULL, NULL, NULL, NULL, $value);
  }
}

unset($tempoid);unset($humoid);unset($mainsoid);

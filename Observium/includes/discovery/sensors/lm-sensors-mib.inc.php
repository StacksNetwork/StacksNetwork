...
foreach ($lm_array['temp'] as $index => $entry)
{
  ...
  $value = $entry['lmTempSensorsValue'] / $divisor;
  if ($entry['lmTempSensorsValue'] > 0 && $value <= 1000)
  {
    discover_sensor($valid['sensor'], 'temperature', $device, $oid, $index, 'lmsensors', $descr, $divisor, 1, NULL, NULL, NULL, NULL, $value);
  }
}
...
foreach ($lm_array['fan'] as $index => $entry)
{
  ...
  $value = $entry['lmFanSensorsValue'] / $divisor;
  if ($entry['lmFanSensorsValue'] > 0)
  {
    discover_sensor($valid['sensor'], 'fanspeed', $device, $oid, $index, 'lmsensors', $descr, $divisor, 1, NULL, NULL, NULL, NULL, $value);
  }
}
...
foreach ($lm_array['volt'] as $index => $entry)
{
  ...
  $value = $entry['lmVoltSensorsValue'] / $divisor;
  if (is_numeric($entry['lmVoltSensorsValue']))
  {
    discover_sensor($valid['sensor'], 'voltage', $device, $oid, $index, 'lmsensors', $descr, $divisor, 1, NULL, NULL, NULL, NULL, $value);
  }
}

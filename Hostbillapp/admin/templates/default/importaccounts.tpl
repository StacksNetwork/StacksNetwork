						...
						<div class="blu" style="padding: 10px;">
							...
							<form action="" method="post" onsubmit="return getpackages();">
								...
								<select name="server_id" id="server_id" class="inp">
									{foreach from=$servers item=srvr}
										<option value="{$srvr.id}">{$srvr.groupname}: {$srvr.name}{if $srvr.ip} - {$srvr.ip}{elseif $srvr.host} - {$srvr.host}{/if}</option>
									{/foreach}
								</select><br />
								<input type="submit" style="font-weight: bold" value="{$lang.getpackages}" />
								{securitytoken}</form>

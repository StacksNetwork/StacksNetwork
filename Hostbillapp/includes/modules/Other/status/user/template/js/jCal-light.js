...
(function($) {
    ...
    $.jCal = function(target, opt) {
        ...
        for (var sm = 0; sm < opt.showMonths; sm++)
            $(target).append('<div class="jCalMo span4"></div>');
    ...
    function drawCalControl(target, opt) {
        $(target).append(
                '<div class="jCal">' +
                ((opt.ind == 0 && opt.monthSelect) ? '<div class="left" />' : '') +
                '<div class="month">' +
                '<span class="monthYear">' + opt.day.getFullYear() + '</span>' +
                '<span class="monthName">' + opt.ml[opt.day.getMonth()] + '</span>' +
                '</div>' +
                ((opt.ind == (opt.showMonths - 1) && opt.monthSelect) ? '<div class="right" />' : '') +
                '</div>');
    ...
    function drawCal(target, opt) {
        ...
        //var dmbox = $('<div class="jCalMoDays"></div>').appendTo(target);
        for (var d = offsetDayStart, dE = (copt.fd + copt.ld + (7 - offsetDayEnd)); d < dE; d++) {
            var dayt = new Date((new Date(fd.getTime())).setDate(d - (copt.fd - opt.dayOffset))),
            day = $(((d <= (copt.fd - opt.dayOffset)) ?
                    ...
                    ((opt.dCheck(dayt)) ? 'day' : 'invday') +
                    '">' + (d - (copt.fd - opt.dayOffset)) + '</div>'
                    )
                    )).appendTo(target);
            opt.postInsert.call(day, dayt);
            //day.height(day.width()*1.1);
        }

        //$(target).find('div[id^=' + opt.cID + 'd]:first, div[id^=' + opt.cID + 'd]:nth-child(7n+2)').before('<br style="clear:both; font-size:0.1em;" />');
        $(target).find('div[id^=' + opt.cID + 'd]:first, div[id^=' + opt.cID + 'd]:nth-child(7n+2)').before('<div class="jCalWeek clearfix"></div>').each(function(){
            //var stack = $(this).nextUntil('div[id^=c3d]:nth-child(7n+2)').andSelf();
            //$('<div></div>').insertBefore(this).append(stack.detach()); 
            //console.log(this);
            var prev = $(this).prev();
            $(this).nextUntil('.jCalWeek').andSelf().detach().appendTo(prev);
        });
        $(target).find('div[id^=' + opt.cID + 'd_]:not(.invday)').bind("mouseover mouseout click", $.extend({}, opt),
                function(e) {
                    ...
                    if (e.type == 'click') {
                        e.data.day = osDate;
                        e.data.callback.call(this, osDate, di);

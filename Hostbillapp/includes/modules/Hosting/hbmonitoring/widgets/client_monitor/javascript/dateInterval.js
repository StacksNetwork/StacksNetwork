...
DateInterval.prototype.refreshData = function() {
  var self = this;
  $.getJSON(this.url + '&res=stats&type=' + this.type + this.getIntervalQueryString(), function(stats) {
    self.stats = stats;
    self.trigger('refresh-stats');
  });
  $.getJSON(this.url + '&res=stat&type=' + this.type + '&date=' + this.date, function(stat) {
    self.stat = stat;

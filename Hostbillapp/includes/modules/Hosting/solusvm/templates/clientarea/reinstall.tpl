...
<div class="content-bar nopadding">
    ...
    <script type="text/javascript" >
        ...
        {foreach from=$ostemplates item=templates}{foreach from=$templates item=i name=tpls}ostemplates[{$smarty.foreach.tpls.index}]={literal}{{/literal}id:"{$i[0]}",name: "{$i[1]} {if $i[2] && $i[2]>0}( {$i[2]|price:$currency} ){/if}",distro:"{$i.distro}",family:"{$i.family}"{literal}}{/literal};{/foreach}{/foreach}
    </script>
                ...
                <tr>
                    <td colspan="2"><span class="slabel">{$lang.ostemplate}</span>
                        <select style="min-width:250px;" required="required" name="os" id="virtual_machine_template_id" onchange="swapcheck($(this).val())" >
                            ...
                            {foreach from=$ostemplates item=templates}{foreach from=$templates item=templa}{if $templa.family=='linux' && $templa.distro==$distributions.linux[0]}

...
function snmp_gen_auth(&$device)
{
  ...
  switch ($device['snmpver'])
  {
    case 'v3':
      ...
      switch ($device['authlevel'])
      {
        ...
        case 'noAuthNoPriv':
          ...
          break;
        default:
          print_error('错误: ' . $device['authlevel'] . ' : 不支持的SNMPv3认证.');
      }
      break;
    ...
    case 'v1':
      ...
      break;
    default:
      print_error('错误: ' . $device['snmpver'] . ' : 不支持的SNMP版本.');
  }

  $debug_auth = (!$GLOBALS['config']['snmp']['hide_auth'] ? "DEBUG: SNMP 认证选项 = $cmd" : '');

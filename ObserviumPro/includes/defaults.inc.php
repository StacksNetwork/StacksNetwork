...
#$config['base_url']        = "http://observium.stacksnet.com/";    // Not recomend change this, only if autodetect wrong. Used in WUI
#$config['web_url']         = "http://observium.stacksnet.com:80/"; // This same as base_url but used only in cli for example in alerts
...
$config['page_title_prefix'] = "Observium :: 核心网络监控系统";
$config['timestamp_format']  = 'Y-m-d H:i:s';
$config['page_gen']          = 1;
$config['login_message']     = "未经授权的访问或使用将会对使用者进行刑事或民事诉讼.";
...
$config['show_services']           = 1;  // Enable Services on menu - do not enable
...
$config['snmp']['community'][0] = "StacksNetwork"; // Communities to try during adding hosts and discovery
...
$config['frontpage']['minigraphs']['legend']       = true;        // Enable/Disable the legend on custom mini graph view
$config['frontpage']['minigraphs']['title']        = "缩略图描述";
...
$config['enable_ports_etherlike']       = 1; // Enable Polling EtherLike-MIB (doubles interface processing time)
$config['enable_ports_junoseatmvp']     = 1; // Enable JunOSe ATM VC Discovery/Poller
$config['enable_ports_adsl']            = 1; // Enable ADSL-LINE-MIB
$config['enable_ports_poe']             = 1; // Enable PoE stats collection
$config['enable_ports_fdbcount']        = 1; // Enable count of FDB per-port.
...
#$config['rancid_suffix']                = 'stacksnet.com'; // Domain suffix for non-FQDN device names
...
#$config['smokeping']['suffix']          = ".stacksnet.com";
...
#$config['nfsen_suffix']   = "_stacksnet_com";
...
$config['auth_ldap_server'] = "ldap.stacksnet.com";  // LDAP server name, or array of LDAP server names tried in order.
#$config['auth_ldap_ad_domain'] = "ad.stacksnet.com";   // AD domain name (fqdn form), used to determine DCs if server list is unset
...
#$config['auth_ldap_group']  = array("cn=stacksnet,ou=groups,dc=example,dc=com");
$config['auth_ldap_groupbase'] = "ou=groups,dc=stacksnet,dc=com";
...
$config['syslog']['filter'][] = "无法打开输出管道 '/dev/xconsole'";

$config['syslog']['fifo']  = TURE;       // Set this to a FIFO to take input from FIFO
...
$config['enable_libvirt'] = TURE; // Enable Libvirt VM support

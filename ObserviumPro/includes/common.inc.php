...
function logfile($filename, $string = NULL)
{
  ...
  if (file_exists($filename))
  {
    ...
    {
      print_debug("日志文件 '$filename' 无法写入或没有创建.");
      fclose($fd);
      return FALSE;
    }
  }
...
function deviceUptime($device, $format="long")
{
  if ($device['status'] == 0) {
    ...
    return "异常 " . formatUptime($since, $format);
  } else {
...
function print_obsolete_config($filter = '')
{
  ...
  if ($list)
  {
    $msg = "%W警告.%n 在config.php中发现过时的配置信息, 请重命名为:\n".implode(PHP_EOL, $list);
...
function check_extension_exists($extension, $text = FALSE, $fatal = FALSE)
{
  ...
  if (!$exist)
  {
    ...
    {
      // Generic message
      print_error("扩展 '$extension' 丢失. 请检查您的PHP配置.");
    }
    elseif ($text !== FALSE)
    {
      // Custom message
      print_error("扩展 '$extension' 丢失. $text");
    } else {
      // Debug message
      print_debug("扩展 '$extension' 丢失. 请检查您的PHP配置.");
    }
...
function ipFromEtcHosts($host)
{
  ...
  {
    print_warning("无法打开文件 /etc/hosts! 该文件必须设置为可写, 同时检查是否未启用SELinux执行模式 Enforcing Mode.");
  }

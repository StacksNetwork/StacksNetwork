...
class Net_IPv4
{
    ...
    function parseAddress($address)
    {
        ...
        if (strchr($address, "/")) {
            ...
            if (! $myself->validateIP($parts[0])) {
                return PEAR::raiseError("无效的IP地址");
            }
            $myself->ip = $parts[0];
             ...
             */
                ...
                if (! $myself->validateNetmask($parts[1])) {
                    return PEAR::raiseError("无效的子网掩码");
                }
    ...
    function calculate()
    {
        ...
        if (! is_a($this, "net_ipv4")) {
            $myself = new Net_IPv4;
            return PEAR::raiseError("无法计算未初始化的 Net_IPv4 类型");
        }
        ...
        if (strlen($this->ip)) {
           ...
           return PEAR::raiseError("没有指定IP地址");
        }
        ...
        if (strlen($this->bitmask)) {
            ...
            return PEAR::raiseError("子网掩码或位掩码需要对应计算方法");
        }
	...
	function inSameSubnet($ip1, $ip2)
	{
		if (! is_object($ip1) || strcasecmp(get_class($ip1), 'net_ipv4') <> 0) {
			...
			if (PEAR::isError($ipobj)) {
                return PEAR::raiseError("IP地址必须为可理解的格式或是 Net_IPv4 对象");
			}
		}

...
class ref{
  ...
  public function __construct($format = 'html'){
    ...
    if($format instanceof RFormatter){
      ...
      if(!class_exists($format, false))
        throw new \Exception(sprintf('%s 类别未找到', $format));
  ...
  public function __get($name){
    throw new \Exception(sprintf('没有这种属性: %s', $name));
  }
  ...
  public function __set($name, $value){
    throw new \Exception(sprintf('无法设置 %s. 不允许', $name));
  }  
  ...
  public static function splitRegex($pattern){
    ...
    if(!$maybeValid)
      throw new \Exception('模式似乎不是有效的PHP正则表达式');
    ...
    foreach($matches as $m){
      ...
      if($m[0] === '['){
        ...
        foreach($ccTokens as $i => $cm) {
          ...
          if($cm[0] === '\\'){
            ...
            if(preg_match('/^\\\\[dsw]$/i', $cm)) {
              ...
              throw new \Exception('不完全的regex令牌');
        ...
        if(strlen($m) === 2)
          throw new \Exception('无效或不受支持的组类型');
        ...
        if($lastType === 1 || ($lastType === 2 && !count($openGroups)))
          throw new \Exception('在这里空替代有效地截断正则表达式');
  ...
  public static function config($key, $value = null){
    ...
    if(!array_key_exists($key, static::$config))
      throw new \Exception(sprintf('无法识别的选项: "%s". 可用的选项是: %s', $key, implode(', ', array_keys(static::$config))));
  ...
  protected function fromReflector(\Reflector $reflector, $single = '', \Reflector $context = null){
    ...
    foreach($items as $item){
      ...
      if($item->isInternal()){
        ...
        $meta['title'] = ($extension instanceof \ReflectionExtension) ? sprintf('内部 - 分类 %s (%s)', $extension->getName(), $extension->getVersion()) : '内部';
        ...
        $meta['sub'][] = array('定义', basename($item->getFileName()) . ':' . $item->getStartLine());        
      }
      ...
      if(($item instanceof \ReflectionFunction) || ($item instanceof \ReflectionMethod)){
        if(($context !== null) && ($context->getShortName() !== $item->getDeclaringClass()->getShortName()))
          $meta['sub'][] = array('继承自', $item->getDeclaringClass()->getShortName());

        if($item instanceof \ReflectionMethod){
          try{
            $proto = $item->getPrototype();
            $meta['sub'][] = array('原型定义', $proto->class);
          }catch(\Exception $e){}
        }  
  ...
  protected function evaluate(&$subject, $specialStr = false){
      ...
      case 'resource':
        ...
        switch($resType){
          ...
          case 'mysql link persistent':
            ...
            $meta = array(
              'host'             => ltrim(@mysql_get_host_info ($subject), 'MySQL主机信息: '),
    ...
    if(static::$config['showIteratorContents'] && $reflector->isIterateable()){
      ...
      $this->fmt->sectionTitle(sprintf('内容 (%d)', count($itContents)));          
      ...
      foreach($itContents as $key => $value){
        ...
        $this->fmt->startRow();
        $this->fmt->text(array('key', 'iterator'), $key, sprintf('迭代器密钥: %s', $keyInfo));
        $this->fmt->colDiv();
        $this->fmt->sep('=>');
        $this->fmt->colDiv();
        $this->evaluate($value);
    ...
    if($props){
      ...
      foreach($props as $idx => $prop){
        ...
        if($meta){
          if($inherited)
            $meta['sub'] = array(array('宣告', $sourceClass->getShortName()));

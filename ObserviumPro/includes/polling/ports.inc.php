...
if (isset($config['port_descr_parser']) && is_file($config['install_dir'] . "/" . $config['port_descr_parser']))
{
  ...
  if (function_exists('custom_port_parser'))
  {
    ...
    print_warning("警告. 重写你的自定义端口分析器文件 [".$config['install_dir'] . "/" . $config['port_descr_parser']."], 使用函数 custom_port_parser().");
...
foreach ($port_stats as $ifIndex => $port)
{
  if (is_port_valid($port, $device))
  {
    if (!is_array($ports[$port['ifIndex']]))
    {
      ...
      echo("添加: ".$port['ifName']."(".$ifIndex.")(".$ports[$port['ifIndex']]['port_id'].")");
    } elseif ($ports[$ifIndex]['deleted'] == "1") {
      dbUpdate(array('deleted' => '0'), 'ports', '`port_id` = ?', array($ports[$ifIndex]['port_id']));
      log_event("接口已删除标记已移除", $device, 'port', $ports[$ifIndex]);
    ...
    if (isset($ports[$port['ifIndex']]) && $ports[$port['ifIndex']]['deleted'] != "1")
    {
      ...
      log_event("接口被标记为已删除", $device, 'port', $ports[$ifIndex]);
...
foreach ($ports as $port)
{
  ...
  { // Check to make sure Port data is cached.
    ...
    if ($device['os'] == "vmware" && preg_match("/设备 ([a-z0-9]+) 在 .*/", $this_port['ifDescr'], $matches)) { $this_port['ifDescr'] = $matches[1]; }
...
#    }
    ...
    if ($device['snmp_version'] != 'v1')
    {
      ...
      else if ($has_ifXEntry && $port_has_64bit && !$port['port_64bit'])
      {
        ...
        $port['port_64bit'] = 1;
        $port['update']['port_64bit'] = 1;
        log_event('接口更改: [HC] -> Counter64 (可能的原因 可支配的峰值)', $device, 'port', $port);
      }
    ...
    foreach ($data_oids as $oid)
    {
      if ($port[$oid] != $this_port[$oid])
      {
        ...
        if ($oid == 'ifOperStatus' && ($port[$oid] == 'up' || $port[$oid] == 'down') && isset($this_port[$oid]))
        {
          ...
          log_event('接口 '.ucfirst($this_port[$oid]) . ": [$oid] " . $port[$oid] . ' -> ' . $this_port[$oid], $device, 'port', $port, 'warning');
        } else {
          $log_event[] = $msg;
        }
    ...
    if ((bool)$log_event) { log_event('接口更改: ' . implode('; ', $log_event), $device, 'port', $port); }
    ...
    if ($custom_port_parser)
    {
      ...
      if ((bool)$log_event) { log_event('接口更改 (属性): ' . implode('; ', $log_event), $device, 'port', $port); }
    }
    // End parse ifAlias
    ...
    foreach ($stat_oids_db as $oid)
    {
      ...
      if (isset($port[$oid]))
      {
        ...
        if ($oid_rate < 0)
        {
          print_warning("负的 $oid. 可能在下一次的峰值!");
         ...
         isset($port['stats']['ifInOctets_negative_rate'])       || isset($port['stats']['ifOutOctets_negative_rate'])))
    {
      ...
      print_warning("峰值以上 ifSpeed 或者负率检测! 在这里查看 debug 信息: ");
      ...
      $debug_msg .= sprintf("+%'-68s+\n", '');
      $debug_msg .= sprintf("| %-20s | %-20s | %-20s |\n", '轮询时间', 'if'.$hc_prefix.'OutOctets', 'if'.$hc_prefix.'InOctets');
    ...
    if ($this_port['pagpOperationMode'] || $port['pagpOperationMode'])
    {
      ...
      if ((bool)$log_event) { log_event('接口变更 (端口聚集协议): ' . implode('; ', $log_event), $device, 'port', $port); }
    }
    // End Update PAgP
...
#    if ($device['os'] == "aos") { include("port-alcatel.inc.php"); }
    ...
    if (count($port['update']))
    {
      ...
      print_debug("端口已更新 rows=$updated");
    }
    ...
    if (count($port['state']))
    {
      if (empty($port['poll_time']))
      {
        ...
        if ($insert === FALSE) { print_error("某些内存数据库错误在表 'ports-state'."); }
        else { print_debug("状态插入 port_id=".$port['port_id']); }
      }
      $updated = dbUpdate($port['state'], 'ports-state', '`port_id` = ?', array($port['port_id']));
      print_debug("状态已更新 rows=$updated");
    }
  ...
  elseif ($port['disabled'] != "1")
  {
    ...
    if (isset($port['ifIndex']) && $port['deleted'] != "1")
    {
      ...
      log_event("接口标记为已删除", $device, 'port', $port);
    }
  } else {
    print_message("端口已禁用.");
  }

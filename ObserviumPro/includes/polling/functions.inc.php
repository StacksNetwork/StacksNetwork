...
function poll_sensor($device, $class, $unit, &$oid_cache)
{
  ...
  {
    echo("检测 (" . $sensor['poller_type'] . ") $class " . $sensor['sensor_descr'] . " ");
    ...
    if ($sensor['state_event'] != 'ignore')
    {
      if (!$sensor['sensor_state'])
      {
        ...
        {
          $msg  = ucfirst($class) . " 警报: " . $device['hostname'] . " " . $sensor['sensor_descr'] . " 赋值之内: " . $sensor_value . "$unit (< " . $sensor['sensor_limit_low'] . "$unit)";
          log_event(ucfirst($class) . ' ' . $sensor['sensor_descr'] . " 赋值之内: " . $sensor_value . " $unit (< " . $sensor['sensor_limit_low'] . " $unit)", $device, $class, $sensor['sensor_id']);
        }
        else if ($sensor['sensor_limit'] != "" && $sensor['sensor_value'] <= $sensor['sensor_limit'] && $sensor_value > $sensor['sensor_limit'])
        {
          $msg  = ucfirst($class) . " 警报: " . $device['hostname'] . " " . $sensor['sensor_descr'] . " 超过阈值: " . $sensor_value . "$unit (> " . $sensor['sensor_limit'] . "$unit)";
          log_event(ucfirst($class) . ' ' . $sensor['sensor_descr'] . " 高于阈值: " . $sensor_value . " $unit (> " . $sensor['sensor_limit'] . " $unit)", $device, $class, $sensor['sensor_id']);
        }
      }
      else if ($sensor_new['state_event'] != $sensor['state_event'] && $sensor['state_event'] != '')
      {
        ...
        {
          case 'alert':
            $msg  = ucfirst($class) . " 警报: " . $device['hostname'] . " " . $sensor['sensor_descr'] . " is under ALERT state: " . $sensor_state_name . " (以前的状态: " . $sensor['state_name'] . ")";
            log_event($msg, $device, $class, $sensor['sensor_id']);
            break;
          case 'warning':
            $msg  = ucfirst($class) . " 警告: " . $device['hostname'] . " " . $sensor['sensor_descr'] . " in WARNING state: " . $sensor_state_name . " (以前的状态: " . $sensor['state_name'] . ")";
            log_event($msg, $device, $class, $sensor['sensor_id']);
            break;
          case 'up':
            $msg  = ucfirst($class) . " 正常: " . $device['hostname'] . " " . $sensor['sensor_descr'] . " in NORMAL state: " . $sensor_state_name . " (以前的状态: " . $sensor['state_name'] . ")";
            log_event($msg, $device, $class, $sensor['sensor_id']);
            break;
        }
      }
    } else {
      print_message("[%y传感器被忽略%n]", 'color');
    }
    echo("\n");
    ...
    // Enable graph
    $graphs[$sensor['sensor_class']] = TRUE;
...
{
  global $config, $debug, $device, $polled_devices, $db_stats, $memcache, $exec_status, $alert_rules, $alert_table, $graphs, $attribs;
  ...
  if (!is_dir($host_rrd_dir)) { mkdir($host_rrd_dir); echo("创建的目录 : $host_rrd_dir\n"); }
  ...
  if ($device['status'] != $status)
  {
    ...
    dbInsert(array('importance' => '0', 'device_id' => $device['device_id'], 'message' => "设备是 " .($status == '1' ? 'up' : 'down')), 'alerts');

    log_event('设备状态更改为 ' . ($status == '1' ? '正常' : '异常') . ($status_type != '' ? ' (' . $status_type . ')' : ''), $device, 'system');
  }
  ...
  if ($status == "1")
  {
    ...
    if ($debug) { echo("更新 " . $device['hostname'] . " - ");print_vars($update_array);echo(" \n"); }
...
function collect_table($device, $oids_def, &$graphs)
{
  ...
  if ($use_walk)
  {
    ...
    {
      print_debug("  警告, 表 '".$oids_def['table']."' for '$mib' 禁用和跳过.");
  ...
  if (!$use_walk && !$oids_ok)
  {
    print_debug("  警告, oids '".implode("', '", array_keys($oids_def['oids']))."' 为 '$mib' 禁用和跳过.");
  ...
  {
    case 'snmpwalk_cache_oid':
      $data = snmpwalk_cache_oid($device, $oids_def['table'], (), $mib, $mib_dirs);
      break;array
    case 'snmp_get_multi':
  ...
  if (isset($GLOBALS['exec_status']['exitcode']) && $GLOBALS['exec_status']['exitcode'] !== 0)
  {
    ...
    print_debug("  警告, 最新 snmp walk/get 返回不良的出口状态 '$mib', RRD 更新跳过.");
    return FALSE;
  }
  ...
  if (isset($rrd['ok']) && $rrd['ok'])
  {
    ...
    if ($GLOBALS['debug'])
    {
      ...
      if ($in_args)
      {
        print_message("%r警告%n, 在文件 '%W".$rrd_file_info['filename']."%n' 不同的DS列表. 没有: ".implode(', ', $in_args));
      }
      $in_file = array_diff($ds_list, $rrd['ds_list']);
      if ($in_file)
      {
        print_message("%r警告%n, 在文件 '%W".$rrd_file_info['filename']."%n' 不同的DS列表. 过量: ".implode(', ', $in_file));
      }
  ...
  else if ($use_walk)
  {
    ...
    print_debug("  警告, table '".$oids_def['table']."' for '$mib' 禁用.");
  } else {
    ...
    print_debug("  警告, oids '".implode("', '", array_keys($oids_def['oids']))."' for '$mib' 禁用.");
  }

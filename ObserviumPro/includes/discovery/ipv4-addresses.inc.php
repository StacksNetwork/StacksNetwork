    ...
    foreach ($addresses as $ipv4_address => $entry)
    {
      ...
      if (isset($old_table[$ifIndex][$ipv4_address]))
      {
        ...
        if (count($update_array))
        {
          ...
          {
            log_event("IPv4移除: $ipv4_address/".$old_table[$ifIndex][$ipv4_address]['ipv4_prefixlen'], $device, 'port', $old_table[$ifIndex][$ipv4_address]['port_id']);
            log_event("IPv4添加: $full_address", $device, 'port', $port_id);
          } else {
            log_event("IPv4更改: $ipv4_address/".$old_table[$ifIndex][$ipv4_address]['ipv4_prefixlen']." -> $full_address", $device, 'port', $port_id);
          }
          echo('U');
...
foreach (dbFetchRows($query, array($device_id)) as $entry)
{
  ...
  if (!isset($valid[$ip_version][$valid_address]))
  {
    ...
    log_event("IPv4移除: $full_address", $device, 'port', $port_id);
    echo('-');

...
if ($ipNetToPhysicalPhysAddress_oid)
{
  ...
  print_debug("使用了 IP-MIB::ipNetToPhysicalPhysAddress");
} else {
  $oid_data = '';
  if ($device['os_group'] == 'cisco')
  {
    ...
    if ($cInetNetToMediaPhysAddress_oid)
    {
      ...
      print_debug("使用了 CISCO-IETF-IP-MIB::cInetNetToMediaPhysAddress");
    }
  } else {
    ...
    if ($ipv6NetToMediaPhysAddress_oid)
    {
      ...
      print_debug("使用了 IPV6-MIB::ipv6NetToMediaPhysAddress");
    }
  }
}
if (!strstr($oid_data, 'ipv4'))
{
  ...
  if ($ipNetToMediaPhysAddress_oid)
  {
    ...
    print_debug("使用了 IP-MIB::ipNetToMediaPhysAddress");
  }
}
...
foreach (explode("\n", $oid_data) as $data)
{
  ...
  if ($ip & $port_id)
  {
    ...
    if (isset($old_table[$if][$ip_version][$ip]))
    {
      ...
      {
        print_debug("更改MAC地址 $ip 从 $old_mac 到 $clean_mac");
        log_event("MAC地址变更: $ip : " . format_mac($old_mac) . " -> " . format_mac($clean_mac), $device, "port", $port_id);
...
foreach ($cache_arp as $entry)
{
  ...
  if (!isset($mac_table[$entry_if][$entry_version][$entry_ip]))
  {
    ...
    print_debug("删除MAC地址 $entry_mac 对于 $entry_ip");
    //log_event("MAC地址已删除: $entry_ip : " . format_mac($entry_mac), $device, "port", $entry['port_id']);
    echo("-");
  }
}

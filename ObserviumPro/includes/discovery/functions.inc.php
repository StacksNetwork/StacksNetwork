...
function discover_storage(&$valid, $device, $storage_index, $storage_type, $storage_mib, $storage_descr, $storage_units, $storage_size, $storage_used, $storage_hc = 0)
{
  ...
  if (!isset($storage_db['storage_id']))
  {
    ...
    echo('+');
    log_event("存储已添加: index $storage_index, mib $storage_mib, descr $storage_descr", $device, 'storage', $id);
  } else {
    $update = array();
    foreach ($params as $param)
    {
    ...
    if (count($update))
    {
      ...
      echo('U');
      log_event("存储已更新: index $storage_index, mib $storage_mib, descr $storage_descr", $device, 'storage', $storage_db['storage_id']);
    } else {
      echo('.');
    }
  }
...
function discover_mempool(&$valid, $device, $mempool_index, $mempool_mib, $mempool_descr, $mempool_precision = 1, $mempool_total, $mempool_used, $mempool_hc = 0)
{
  ...
  if (!isset($mempool_db['mempool_id']))
  {
    ...
    echo('+');
    log_event("内存池已添加: mib $mempool_mib, index $mempool_index, descr $mempool_descr", $device, 'mempool', $id);
  } else {
    $update = array();
    foreach ($params as $param)
    {
    ...
    if (count($update))
    {
      ...
      echo('U');
      log_event("内存池已更新: mib $mempool_mib, index $mempool_index, descr $mempool_descr", $device, 'mempool', $mempool_db['mempool_id']);
    } else {
      echo('.');
    }
  }
...
function discover_toner(&$valid, $device, $toner_oid, $toner_index, $toner_type, $toner_descr, $toner_capacity_oid = NULL, $toner_capacity = NULL, $toner_current = NULL)
{
  ...
  if (!isset($toner_db['toner_id']))
  {
    ...
    echo('+');
    log_event("硒鼓已添加: type $toner_type, index $toner_index, descr $toner_descr", $device, 'toner', $id);
  } else {
    $update = array();
    foreach ($params as $param)
    {
    ...
    if (count($update))
    {
      ...
      echo('U');
      log_event("硒鼓已更新: type $toner_type, index $toner_index, descr $toner_descr", $device, 'toner', $toner_db['toner_id']);
    } else {
      echo('.');
    }
  }

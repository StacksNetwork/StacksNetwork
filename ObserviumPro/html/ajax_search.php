...
if (isset($_REQUEST['queryString']))
{
  ...
  if (strlen($queryString) > 0)
  {
    ...
    {
      $found = 1;
      echo('<li class="nav-header">找到设备: '.count($results).'</li>' . PHP_EOL);
    ...
    {
      $found = 1;
      echo('<li class="nav-header">找到端口: '.count($results).'</li>');
    ...
    {
      $found = 1;
      echo('<li class="nav-header">找到传感器: '.count($results).'</li>');
    ...
    {
      $found = 1;
      echo('<li class="nav-header">找到AP: '.count($results).'</li>');
...
#      foreach ($addr_ports as $port_id => $result)
    ...
    if (!$found)
    {
      echo('<li class="nav-header">没有搜索结果.</li>');
    }
  } // There is a queryString.
} else {
  echo('<li class="nav-header">不可接进入该脚本! 请重新加载该页面.</li>');
}

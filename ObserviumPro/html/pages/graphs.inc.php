...
if (!$auth)
{
  ...
  $navbar = array('brand' => "图像", 'class' => "navbar-narrow");
  ...
  switch ($type)
  {
    ...
    default:
      ...
      $navbar['options']['graph']     = array('text' => '图像');
...
$navbar['options']['previous'] =  array('text' => '过往视图');
$navbar['options']['trend']    =  array('text' => '趋势视图');
$navbar['options']['max']      =  array('text' => '最大化视图');

$navbar['options_right']['showcommand'] =  array('text' => 'RRD命令');
...
?>
  ...
  <div class="well info_box">
    ...
    <div class="content">
      <?php echo("RRDTool输出: ".$return."<br />"); ?>
      <?php echo("<p>总需时: ".$graph_return['total_time']." | RRDTool时间: ".$graph_return['rrdtool_time']."s</p>"); ?>
    </div>
  </div>

  <div class="well info_box">
    <div class="title">
      <i class="oicon-application-terminal"></i> RRDTool指令
    </div>
    <div class="content">
  ...
  <div class="well info_box">
    <div class="title">
      <i class="oicon-database"></i> RRDTool文件使用
    </div>
    <div class="content">
      <?php
        if (is_array($graph_return['rrds']))
        {
          foreach ($graph_return['rrds'] as $rrd)
          {
            ...
            echo("无RRD信息返回. 这可能是因为该图模块不知道返回该数据. <br />");
        }
      ?>
    </div>
  </div>

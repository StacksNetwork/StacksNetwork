...
<?php
  ...
  function show_traffic($config)
  {
    ...
    {
      unset ($separator);
      foreach (dbFetchRows("SELECT * FROM ports as I, devices as D WHERE I.device_id = D.device_id AND I.deleted = 0 ORDER BY I.ifAlias") as $interface)
      {
        if ($interface['port_descr_type'] == 'transit')
        {
          $ports['transit'] .= $separator . $interface['port_id'];
          $separator = ",";
        }
        unset ($separator);
        if ($interface['port_descr_type'] == 'peering')
        {
          $ports['peering'] .= $separator . $interface['port_id'];
          $separator = ",";
        }
        unset ($separator);
        if ($interface['port_descr_type'] == 'core')
        {
  ...
  function show_status($config)
  {
    ...
    echo('    <h3 class="bill">设备报警</h3>' . PHP_EOL);
  ...
  function show_eventlog($config)
  {
    ...
    echo('<div class="row">' . PHP_EOL);
    echo('  <div class="col-md-12">' . PHP_EOL);
    echo('    <h3 class="bill">最近的事件日志条目</h3>' . PHP_EOL);

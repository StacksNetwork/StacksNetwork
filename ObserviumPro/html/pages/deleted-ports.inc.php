...
echo('<table class="table table-striped table-bordered table-condensed">
  <thead><tr>
    ...
    <th>描述</th>
    <th>删除自</th>
    <th style="text-align: right;"><a href="'.generate_url(array('page'=>'deleted-ports', 'purge'=>'all')).'"><button class="btn btn-danger btn-small"><i class="icon-remove icon-white"></i> 清除全部</button></a></th>
  </tr></thead>');
...
foreach (dbFetchRows('SELECT * FROM `ports` AS P, `devices` as D WHERE P.`deleted` = "1" AND D.device_id = P.device_id') as $port)
{
  ...
  if (port_permitted($port['port_id'], $port['device_id']))
  {
    ...
    echo('<td style="width: 100px;"><a href="'.generate_url(array('page'=>'deleted-ports', 'purge'=>$port['port_id'])).'"><button class="btn btn-danger btn-small"><i class="icon-remove icon-white"></i> 清除</button></a></td>');
  }
}

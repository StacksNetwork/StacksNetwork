...
echo("    <tr>");
echo("      <th>服务器</th>");
echo("      <th style=\"width: 250px;\">地址</th>");
echo("      <th style=\"width: 200px;\">类型</th>");
echo("      <th style=\"width: 130px;\">状态</th>");
echo("      <th style=\"width: 130px;\">流量</th>");
echo("    </tr>");
echo("   </thead>");
...
foreach (dbFetchRows("SELECT * FROM `netscaler_vservers` WHERE `device_id` = ? AND `vsvr_id` = ? ORDER BY `vsvr_label`", array($device['device_id'], $vars['vsvr'])) as $vsvr)
{
  ...
  echo("<td><span class='".$vsvr_class."'>" . $vsvr['vsvr_state'] . "</span><br />".$vsvr['num_services']." 服务</td>");
...
echo('    <tr>');
echo('      <th>服务器</th>');
echo('      <th width=250>地址</th>');
echo('      <th width=200>类型</th>');
echo('      <th width=130>状态</th>');
echo('      <th width=130>流量</th>');
echo('    </tr>');
echo('  </thead>');
$i = "0";

...
<table class="table table-hover table-striped-two table-bordered table-condensed table-rounded" style="margin-top: 10px;">
  <thead>
      <tr>
        <th style="width: 250px;"><span style="font-weight: bold;" class="interface">说明</span></th>
...
<?php
  ...
  foreach ($ports as $port)
  {
    ...
    $port['ifAlias'] = str_ireplace("[PNI]", "私有", $port['ifAlias']);
...
#            <span class=small style='float: left;'>".generate_device_link($port)." ".generate_port_link($port)." </span>");
    ...
    {
      echo("<span style='float: right;'><a href='device/device=".$port['device_id']."/tab=port/port=".$port['port_id']."/view=macaccounting/'><img src='images/16/chart_curve.png' align='absmiddle'> MAC核算</a></span>");
    }

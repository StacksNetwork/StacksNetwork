    ...
    if ($value != '')
    {
      switch ($var)
      {
        ...
        case 'cbqos':
          if ($value && $value != 'no')
          {
            $where[] = generate_query_values($cache['ports']['cbqos'], 'ports.port_id');
          }
          break;
      }
    }
...
function bug()
{
  ...
  <button type="button" class="close" data-dismiss="alert">&次;</button>
  <strong>Bug!</strong> 请报告给 Observium 开发团队.
</div>');
}
...
function pagination(&$vars, $total, $return_vars = FALSE)
{
  ...
  if ($total > 9)
  {
    ...
    $pagination .= '<div class="col-sm-1"><span class="btn disabled" style="line-height: 20px;">'.$total.'&nbsp;条数据</span></div>';
    ...
    if ($page < $counter - 1)
    {
      ...
      #$pagination.= "<li><a href='".generate_url($vars, array('pageno' => $lastpage))."'>末页</a></li>";
    }
    else if ($lastpage > 1)
    {
      $pagination.= "<li class='active'><a>下一页</a></li>";
      #$pagination.= "<li class='active'><a>末页</a></li>";
    }
      ...
      if ($value != '')
      {
        switch ($var)
        {
          case 'v':
            ...
            break;
          case 'feed':
            $title = "Observium :: ".ucfirst($value)." 进入";
...
{
  if ($message == "设备状态变化为异常") { $icon = "server_connect.png"; }
  if ($message == "设备状态变化为正常") { $icon = "server_go.png"; }
  if ($message == "接口变为异常" || $message == "接口状态变化为异常") { $icon = "if-disconnect.png"; }
  if ($message == "接口变为正常" || $message == "接口状态变化为正常") { $icon = "if-connect.png"; }
  if ($message == "接口禁用") { $icon = "if-disable.png"; }
  if ($message == "接口启用") { $icon = "if-enable.png"; }
...
function get_rancid_filename($hostname, $rdebug)
{
  ...
  {
    $hostnames[] = $shortname;
    if ($rdebug) { echo("主机名不同于短主机名, 寻找二者<br />"); }
  }
  ...
  if (isset($config['rancid_suffix']) && $config['rancid_suffix'] !== '')
  {
    ...
    if ($rdebug) { echo("RANCID 后缀已配置, 同时寻找 " . $hostname . '.' . trim($config['rancid_suffix'], ' .') . "<br />"); }
  }

  foreach ($config['rancid_configs'] as $config_path)
  {
    if ($config_path[strlen($config_path)-1] != '/') { $config_path .= '/'; }
    if ($rdebug) { echo("在配置目录寻找: <b>$config_path</b><br />"); }

    foreach ($hostnames as $host)
    {
      if (is_file($config_path . $host))
      {
        if ($rdebug) { echo("文件 <b>" . $config_path . $host . "</b> 已找到.<br />"); }
...
function get_smokeping_files($rdebug = 0)
{
  ...
  if ($rdebug) { echo('- 经过检索 ' . $config['smokeping']['dir'] . '<br />'); }
  ...
  if (is_dir($config['smokeping']['dir']))
  {
    foreach (new RecursiveIteratorIterator(new RecursiveDirectoryIterator($config['smokeping']['dir'])) as $file)
    {
      ...
      {
        if ($rdebug) { echo('- 找到的文件结束 ".rrd": ' . basename($file) . '<br />'); }

        if (strstr($file, "~"))
        {
          list($target,$slave) = explode("~", basename($file,".rrd"));
          if ($rdebug) { echo('- 确定为目标从文件 <b>' . $target . '</b><br />'); }
          $target = str_replace($config['smokeping']['split_char'], ".", $target);
          if ($config['smokeping']['suffix']) { $target = $target.$config['smokeping']['suffix']; if ($rdebug) { echo('- 后缀已配置, 现在的目标 <b>' . $target . '</b><br />'); } }
          $smokeping_files['incoming'][$target][$slave] = $file;
          $smokeping_files['outgoing'][$slave][$target] = $file;
        } else {
          $target = basename($file,".rrd");
          if ($rdebug) { echo('- 被确定为一个本地文件, 目标 <b>' . $target . '</b><br />'); }
          $target = str_replace($config['smokeping']['split_char'], ".", $target);
          if ($rdebug) { echo('- 更换后的配置 split_char ' . $config['smokeping']['split_char'] . ' by . 目标为 <b>' . $target . '</b><br />'); }
          if ($config['smokeping']['suffix']) { $target = $target.$config['smokeping']['suffix']; if ($rdebug) { echo('- 后缀配置, 现在的目标是 <b>' . $target . '</b><br />'); } }
    ...
    if ($rdebug) { echo("- Smokeping RRD 目录未找到: " . $config['smokeping']['dir']); }
  }

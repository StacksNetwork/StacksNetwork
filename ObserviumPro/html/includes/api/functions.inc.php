...
function api_load_module($module) {
  ...
  api_show_debug("加载模块", array("module"=>$module, "file"=>$file));
  return $res;
}
...
function api_errorcodes($code, $section="error") {
  global $errorcodes;
  //api_show_debug("所有错误代码", $errorcodes);
...
function api_authenticate_user($username, $password)
{
	...
	{
		print_error(错误: 没有定义的有效 auth_mechanism !');
		exit();
	}
...
function api_device_permitted($device_id) {
  ...
  if ($vars['user']['level'] >= 10) {
    ...
    api_show_debug("检查设备权限", $vars['device']);

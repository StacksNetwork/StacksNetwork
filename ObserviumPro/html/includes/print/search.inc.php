...
function print_search($data, $title = NULL, $button = 'search', $url = NULL)
{
  ...
  switch($button)
  {
    ...
    case 'update':
      $string .= '<i class="icon-refresh"></i> 更新</button>' . PHP_EOL;
      break;
    default:
      $string .= '<i class="icon-search"></i> 搜索</button>' . PHP_EOL;
  }
  $string .= '    </div>' . PHP_EOL;
  $string .= '</div></div></div></form>' . PHP_EOL;
  $string .= '<!-- 结束搜索表单 -->' . PHP_EOL . PHP_EOL;
  ...
  switch($item['type'])
  {
    ...
    case 'datetime':
      ...
      if ($item['from'] !== FALSE)
      {
        ...
        $string .= '    <span class="add-on btn"><i data-time-icon="icon-time" data-date-icon="icon-calendar"></i> 从</span>' . PHP_EOL;
      ...
      if ($item['to'] !== FALSE)
      {
        ...
        $string .= '    <span class="add-on btn"><i data-time-icon="icon-time" data-date-icon="icon-calendar"></i> 至</span>' . PHP_EOL;
        $string .= '    <input type="text" class="input-medium" data-format="yyyy-MM-dd hh:mm:ss" ';
    ...
    case 'select':
      if (empty($item['values'])) { $item['values'] = array(0 => '[没有任何数据]'); }
      ...
      foreach ($item['values'] as $k => $name)
      {
        ...
        if ($name == '[没有任何数据]') { $string .= ' disabled'; }

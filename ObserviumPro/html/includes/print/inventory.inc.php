...
function print_inventory($vars)
{
  ...
  if ($vars['page'] == 'device' && is_numeric($vars['device']) && device_permitted($vars['device']))
  {
    ...
    echo('<div class="btn-group pull-right" style="margin-top:5px; margin-right: 5px;">
      <button class="btn btn-small" onClick="expandTree(\'enttree\');return false;"><i class="icon-plus muted small"></i> 展开</button>
      <button class="btn btn-small" onClick="collapseTree(\'enttree\');return false;"><i class="icon-minus muted small"></i> 折叠</button>
    </div>');
  ...
  if (!$short)
  {
    ...
    $string .= '      <th>名称</th>' . PHP_EOL;
    $string .= '      <th>说明</th>' . PHP_EOL;
    $string .= '      <th>部分 #</th>' . PHP_EOL;
    $string .= '      <th>序列号 #</th>' . PHP_EOL;
    $string .= '    </tr>' . PHP_EOL;
    $string .= '  </thead>' . PHP_EOL;
  }

...
function print_status($status)
{
 ...
 /* $string .= '  <thead>' . PHP_EOL;
  ...
  $string .= '    <th>类型</th>' . PHP_EOL;
  $string .= '    <th>状态</th>' . PHP_EOL;
  $string .= '    <th>实体</th>' . PHP_EOL;
  // $string .= '    <th>位置</th>' . PHP_EOL;
  $string .= '    <th>时间/信息</th>' . PHP_EOL;
  $string .= '  </tr>' . PHP_EOL;
  $string .= '  </thead>' . PHP_EOL;
 ...
 */
  ...
  if ($status['devices'])
  {
    ...
    foreach ($entries as $device)
    {
      ...
      // $string .= '    <td><span class="badge badge-inverse">设备</span></td>' . PHP_EOL;
      $string .= '    <td><span class="label label-important">设备异常</span></td>' . PHP_EOL;
      $string .= '    <td>' . generate_device_link($device, short_hostname($device['hostname'])) . '</td>' . PHP_EOL;
  ...
  if ($status['uptime'])
  {
    if (filter_var($config['uptime_warning'], FILTER_VALIDATE_FLOAT) !== FALSE && $config['uptime_warning'] > 0)
    {
      ...
      foreach ($entries as $device)
      {
        ...
        // $string .= '    <td><span class="badge badge-inverse">设备</span></td>' . PHP_EOL;
        $string .= '    <td><span class="label label-success">设备重启</span></td>' . PHP_EOL;
        $string .= '    <td class="entity"><i class="oicon-servers"></i> ' . generate_device_link($device, short_hostname($device['hostname'])) . '</td>' . PHP_EOL;
        // $string .= '    <td style="white-space: nowrap">' . escape_html(truncate($device['location'], 30)) . '</td>' . PHP_EOL;
  ...
  if ($status['ports'] || $status['links'])
  {
    ...
    foreach ($entries as $port)
    {
      if ($i > $max_count)
      {
        $string .= '  <tr><td></td><td><span class="badge badge-info">端口</span></td>';
        $string .= '<td><span class="label label-important">端口异常</span></td>';
        $string .= '<td colspan=3>过多的异常端口. 查看 <strong><a href="'.generate_url(array('page'=>'ports'), array('state'=>'down')).'">所有异常端口</a></strong>.</td></tr>' . PHP_EOL;
        break;
      }
      humanize_port($port);
      $string .= '  <tr>' . PHP_EOL;
      $string .= '    <td class="entity">' . generate_device_link($port, short_hostname($port['hostname'])) . '</td>' . PHP_EOL;
      // $string .= '    <td><span class="badge badge-info">端口</span></td>' . PHP_EOL;
      $string .= '    <td><span class="label label-important">端口异常</span></td>' . PHP_EOL;
      ...
      $string .= '    <td style="white-space: nowrap">异常状态 ' . formatUptime($config['time']['now'] - strtotime($port['ifLastChange']), 'short'); // This is like deviceUptime()
  ...
  if ($status['errors'])
  {
    ...
    foreach ($entries as $port)
    {
      ...
      $string .= '    <td class="entity">' . generate_device_link($port, short_hostname($port['hostname'])) . '</td>' . PHP_EOL;
      // $string .= '    <td><span class="badge badge-info">端口</span></td>' . PHP_EOL;
      $string .= '    <td><span class="label label-important">端口错误</span></td>' . PHP_EOL;
  ...
  if ($status['services'])
  {
    ...
    foreach ($entries as $service)
    {
      ...
      // $string .= '    <td><span class="badge">服务</span></td>' . PHP_EOL;
      $string .= '    <td><span class="label label-important">服务异常</span></td>' . PHP_EOL;
      ...
      $string .= '    <td style="white-space: nowrap">异常状态 ' . formatUptime($config['time']['now'] - strtotime($service['service_changed']), 'short') . '</td>' . PHP_EOL; // This is like deviceUptime()
      $string .= '  </tr>' . PHP_EOL;
    }
  }
  ...
  if ($status['bgp'])
  {
    if (isset($config['enable_bgp']) && $config['enable_bgp'])
    {
      ...
      foreach ($entries as $peer)
      {
        ...
        $string .= '    <td style="white-space: nowrap"><i class="oicon-chain"></i> ' . $peer_ip . '</td>' . PHP_EOL;
...
function get_status_array($status)
{
  ...
  if ($status['devices'])
  {
    ...
    foreach ($entries as $device)
    {
      $boxes[] = array('sev' => 100, 'class' => '设备', 'event' => '异常', 'device_link' => generate_device_link($device, short_hostname($device['hostname'])),
  ...
  if ($status['uptime'])
  {
    if (filter_var($config['uptime_warning'], FILTER_VALIDATE_FLOAT) !== FALSE && $config['uptime_warning'] > 0)
    {
      ...
      foreach ($entries as $device)
      {
        $boxes[] = array('sev' => 10, 'class' => '设备', 'event' => '重启', 'device_link' => generate_device_link($device, short_hostname($device['hostname'])),
  ...
  if ($status['ports'] || $status['links'])
  {
    ...
    foreach ($entries as $port)
    {
      ...
      humanize_port($port);
      $boxes[] = array('sev' => 50, 'class' => '端口', 'event' => '异常', 'device_link' => generate_device_link($port, short_hostname($port['hostname'])),
  ...
  if ($status['errors'])
  {
    foreach ($cache['ports']['errored'] as $port_id)
    {
      ...
      $boxes[] = array('sev' => 75, 'class' => '端口', 'event' => '错误', 'device_link' => generate_device_link($device, short_hostname($device['hostname'])),
                       'entity_link' => generate_port_link($port, short_ifname($port['label'], 13)),
  ...
  if ($status['services'])
  {
    ...
    foreach ($entries as $service)
    {
      $boxes[] = array('sev' => 50, 'class' => '服务', 'event' => '异常', 'device_link' => generate_device_link($service, short_hostname($service['hostname'])),
  ...
  if ($status['bgp'])
  {
    if (isset($config['enable_bgp']) && $config['enable_bgp'])
    {
      ...
      foreach ($entries as $peer)
      {
        ...
        $boxes[] = array('sev' => 75, 'class' => 'BGP节点', 'event' => '异常', 'device_link' => generate_device_link($peer, short_hostname($peer['hostname'])),

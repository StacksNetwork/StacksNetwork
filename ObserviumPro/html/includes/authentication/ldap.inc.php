...
function ldap_init()
{
  ...
  if (!is_resource($ds))
  {
    print_debug("LDAP[连接到 " . implode(",",$config['auth_ldap_server']) . "]");
    $ds = @ldap_connect(implode(",",$config['auth_ldap_server']), $config['auth_ldap_port']);
    print_debug("LDAP[已连接]");
    ...
    if ($config['auth_ldap_starttls'] && ($config['auth_ldap_starttls'] == 'optional' || $config['auth_ldap_starttls'] == 'require'))
    {
      ...
      {
        session_logout();
        print_error("致命错误: LDAP TLS要求但不成功的谈判 [" . ldap_error($ds) . "]");
        exit;
      }
    }
    ...
    if ($config['auth_ldap_version'])
    {
      ...
      print_debug("LDAP[Version][发送到 " . $config['auth_ldap_version'] . "]");
    }
  }
}
      ...
      if ($config['auth_ldap_kerberized'] || ldap_bind($ds, $binduser, $password))
      {
        ...
        else
        {
          ...
          foreach ($config['auth_ldap_group'] as $ldap_group)
          {
            ...
            if ($compare === -1)
            {
              print_debug("LDAP[Authenticate][比较LDAP错误: " . ldap_error($ds) . "]");
              continue;
            } elseif ($compare === FALSE) {
              print_debug("LDAP[Authenticate][进程组: $ldap_group][不匹配]");
            } else {
              // $compare === TRUE
              print_debug("LDAP[Authenticate][进程组: $ldap_group][匹配]");
              return 1;
...
function ldap_auth_user_level($username)
{
  ...
  if (!isset($cache['ldap']['level'][$username]))
  {
    ...
    foreach ($config['auth_ldap_groups'] as $ldap_group => $ldap_group_info)
    {
      ...
      if ($compare === -1)
      {
        print_debug("LDAP[UserLevel][比较LDAP错误: " . ldap_error($ds) . "]");
        continue;
      } elseif ($compare === FALSE) {
        print_debug("LDAP[UserLevel][进程组: $ldap_group][不匹配]");
      } else {
        // $compare === TRUE
        print_debug("LDAP[UserLevel][进程组: $ldap_group][Level: " . $ldap_group_info['level'] . "]");
        if ($ldap_group_info['level'] > $userlevel)
        {
          $userlevel = $ldap_group_info['level'];
          print_debug("LDAP[UserLevel][允许组级别]");
        } else {
          print_debug("LDAP[UserLevel][忽略组级别]");
        }
      }
    }

    print_debug("LDAP[Userlevel][最终等级: $userlevel]");
...
function ldap_auth_user_id($username)
{
  ...
  if ($entries['count'])
  {
    ...
    print_debug("LDAP[UserID][用户未通过过滤器发现]");
  }
...
function ldap_auth_user_list()
{
  ...
  if ($entries['count'])
  {
    for ($i = 0; $i < $entries['count']; $i++)
    {
      ...
      print_debug("LDAP[UserList][比较: " . implode('|',$config['auth_ldap_group']) . "][".$config['auth_ldap_groupmemberattr']."][$userdn]");
      ...
      foreach ($config['auth_ldap_group'] as $ldap_group)
      {
        ...
        if ($compare === -1)
        {
          print_debug("LDAP[UserList][比较LDAP错误: " . ldap_error($ds) . "]");
          continue;
        } elseif ($compare === FALSE) {
          print_debug("LDAP[UserList][进程组: $ldap_group][不匹配]");
        } else {
          // $$compare === TRUE
          print_debug("LDAP[UserList][验证: $userdn 组 $ldap_group]");
          $authorized = 1;
          break;
        } // FIXME does not support nested groups
      }
...
function ldap_bind_dn($username = "", $password = "")
{
  ...
  if ($bind)
  {
    ...
    print_debug("错误绑定到LDAP服务器: " . $config['auth_ldap_server'] . ": " . ldap_error($ds));
    session_logout();
    return 1;
  }
}
...
function ldap_internal_auth_user_id($result)
{
  ...
  if ($config['auth_ldap_attr']['uidNumber'] == "objectSid")
  {
    ...
    print_debug("LDAP[UserID][转换对象Sid " . ldap_bin_to_str_sid($result['objectsid'][0]) . " 到用户ID " . $userid . "]");
  } else {
    $userid = $result[strtolower($config['auth_ldap_attr']['uidNumber'])][0];
    print_debug("LDAP[UserID][属性 " . $config['auth_ldap_attr']['uidNumber'] . " 批量用户ID " . $userid . "]");
  }

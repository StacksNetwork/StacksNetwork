...
while ($interface = mysql_fetch_assoc($interface_query)) {
 ...
 if ($device['status'] == '1') {
   ...
   $snmp_output = str_replace("该OID上的代理无可用对象", "", $snmp_output);
   $snmp_output = str_replace("该OID上不存在当前的实例", "", $snmp_output);
...
#     echo("Not Updating : " . $device['hostname'] ." $ifDescr ( " . $interface['ifDescr'] . " )\n\n");
   ...
   if ($ifOperStatus == "up") {
    ...
    $snmp_data = str_replace("类型错误 (应该为 Counter32): ","", $snmp_data);
    $snmp_data = str_replace("该OID上不存在当前的实例","", $snmp_data);

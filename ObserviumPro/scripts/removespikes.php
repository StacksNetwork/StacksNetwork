...
foreach($parms as $parameter) {
	...
	case "-R":
		...
		if (!file_exists($rrdfile)) {
			echo "FATAL: 文件 '$rrdfile' 未创建.\n";
			exit(-9);
		}

		if (!is_writable($rrdfile)) {
			echo "FATAL: 文件 '$rrdfile' 此账号不可写.\n";
			exit(-8);
		}
...
if (!$using_cacti) {
	...
	if (strlen($response)) {
		...
		echo "注意: 使用 " . $response_array[0] . " Version " . $response_array[1] . "\n";
	}else{
		echo "FATAL: RRDTool不找到路径. 请确保能在你的路径发现RRDTool!\n";
		exit(-1);
	}
}
...
if ($using_cacti) {
	cacti_log("注意: 去除峰值 '$rrdfile', 方法:'$method'", false, "WEBUI");
}

/* execute the dump command */
echo ($html ? "<tr><td colspan='20' class='spikekill_note'>":"") . "注意: 创建XML文件 '$xmlfile' 从 '$rrdfile'" . ($html ? "</td></tr>\n":"\n");
...
if (file_exists($xmlfile)) {
	...
	if ($using_cacti) {
		echo ($html ? "<tr><td colspan='20' class='spikekill_note'>":"") . "FATAL: RRDtool 命令错误.  请确认路径设置RRDTool是有效的->Paths!" . ($html ? "</td></tr>\n":"\n");
	}else{
		echo ($html ? "<tr><td colspan='20' class='spikekill_note'>":"") . "FATAL: RRDtool 命令错误.  请确保您安装RRDTool是有效的!" . ($html ? "</td></tr>\n":"\n");
	}
	exit(-12);
}
...
if ($backup && !$dryrun) {
	if (copy($rrdfile, $bakfile)) {
		echo ($html ? "<tr><td colspan='20' class='spikekill_note'>":"") . "注意: RRDfile '$rrdfile' 备份到 '$bakfile'" . ($html ? "</td></tr>\n":"\n");
	}else{
		echo ($html ? "<tr><td colspan='20' class='spikekill_note'>":"") . "FATAL: RRDfile 备份 '$rrdfile' 到 '$bakfile' FAILED!" . ($html ? "</td></tr>\n":"\n");
		exit(-13);
	}
}
...
*/
echo ($html ? "<tr><td colspan='20' class='spikekill_note'>":"") . "注意: 搜索XML文件中的峰值 '$xmlfile'" . ($html ? "</td></tr>\n":"\n");
...
if ($method == 1) {
	...
	if ($std_kills) {
		...
		echo ($html ? "<tr><td colspan='20' class='spikekill_note'>":"") . "注意: 没有发现在标准偏差的峰值 '$rrdfile'" . ($html ? "</td></tr>\n":"\n");
	}
}else{
	/* variance subroutine */
	if ($var_kills) {
		...
		echo ($html ? "<tr><td colspan='20' class='spikekill_note'>":"") . "注意: 没有发现方差峰值 '$rrdfile'" . ($html ? "</td></tr>\n":"\n");
	}
}
	...
	if ($total_kills) {
		if (writeXMLFile($new_output, $xmlfile)) {
			if (backupRRDFile($rrdfile)) {
				...
				echo ($html ? "<tr><td colspan='20' class='spikekill_note'>":"") . "FATAL: 无法备份 '$rrdfile'" . ($html ? "</td></tr>\n":"\n");
			}
		}else{
			echo ($html ? "<tr><td colspan='20' class='spikekill_note'>":"") . "FATAL: 无法写入XML文件 '$xmlfile'" . ($html ? "</td></tr>\n":"\n");
		}
	}
}else{
	echo ($html ? "<tr><td colspan='20' class='spikekill_note'>":"") . "注意: 预检请求. 没有更新执行" . ($html ? "</td></tr>\n":"\n");
}
...
function createRRDFileFromXML($xmlfile, $rrdfile) {
	...
	/* execute the dump command */
	echo ($html ? "<tr><td colspan='20' class='spikekill_note'>":"") . "注意: 重导入 '$xmlfile' 到 '$rrdfile'" . ($html ? "</td></tr>\n":"\n");
	if ($using_cacti) {
